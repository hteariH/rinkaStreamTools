...
export const DEFAULTS = {
  ...
  counter: {
    // Счётчик на оверлее: смерти, победы, «сколько раз я это сказал». Считать его
    // можно кнопками в панели, но смысл в горячих клавишах — жать, не отрываясь
    // от игры.
    enabled: true,
    title: "Смертей",
    // Значение — не настройка, а слепок: эфир может оборваться на середине, и
    // терять счёт из-за перезапуска программы обидно.
    value: 0,
    step: 1,
    // «Смертей: -1» выглядит поломкой, а не счётом.
    allowNegative: false,
    theme: "default",
    // Клавиши в именах браузера (KeyboardEvent.code): "F8", "NumpadAdd",
    // "Ctrl+Shift+KeyD". Пусто — хоткея нет, и клавиатуру никто не слушает.
    hotkeys: {
      plus: "",
      minus: "",
      reset: "",
