...
// Опрос медиасессии Windows делает скрипт на PowerShell: в бандл он не попадает —
// его запускает отдельный процесс, а не Node. Путь относительно бинарника тот же,
// что в исходниках, чтобы искать его в двух режимах запуска не пришлось по-разному.
cpSync(
  path.join(root, "src", "nowplaying", "session.ps1"),
  path.join(serverDir, "src", "nowplaying", "session.ps1")
);
